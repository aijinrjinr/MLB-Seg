...
class LA_2D(data.Dataset):
    def __init__(self, root='', suffix='train', resize=[224, 224], transform=None, mlb=False, datasplitpath=''):
        self.root = root
        self.suffix = suffix
    ...
    def __getitem__(self, index):
        ...
        if 'meta' in self.suffix:
            ...
            if 'train' in self.suffix:
                ...
                image_path = self.root + image_name + '/mri_norm2.h5'
                h5f = h5py.File(image_path, 'r')
                images = h5f['image'][:]
                masksgt = h5f['label'][:]
        if 'vali' in self.suffix:
            images = np.transpose(images, (2, 0, 1))
            masksgt = np.transpose(masksgt, (2, 0, 1))
            masks = masksgt
...
def prepare_set2D_mlb(args):

    if args.dataset == 'LA':
        trainset = LA_2D(root=args.train_root, suffix='train', resize=args.size, transform=None, mlb=True,
                         datasplitpath=args.datasplitpath)
        metaset = LA_2D(root=args.meta_root, suffix='meta', resize=args.size, transform=None, mlb=True,
                         datasplitpath=args.datasplitpath)
        testset = LA_2D(root=args.vali_root, suffix='vali', resize=args.size, transform=None, mlb=True,
                        datasplitpath=args.datasplitpath)

    else:
        if  args.dataset == 'Prostate':
            trainset = Prostate_2D(root=args.train_root, suffix='train', resize=args.size, transform=None, mlb=True,
                             datasplitpath=args.datasplitpath)
            metaset = Prostate_2D(root=args.meta_root, suffix='meta', resize=args.size, transform=None, mlb=True,
                            datasplitpath=args.datasplitpath)
            testset = Prostate_2D(root=args.vali_root, suffix='vali', resize=args.size, transform=None, mlb=True,
                            datasplitpath=args.datasplitpath)
...
def prepare_set2D(args):
    if args.dataset == 'LA':
        trainset = LA_2D(root=args.train_root, suffix='train', resize=args.size, transform=None, mlb=False, datasplitpath=args.datasplitpath)
        testset = LA_2D(root=args.vali_root, suffix='vali', resize=args.size, transform=None, mlb=False, datasplitpath=args.datasplitpath)
    else:

        if args.dataset == 'Prostate':
            trainset = Prostate_2D(root=args.train_root, suffix='train', resize=args.size, transform=None,
                                   mlb=False, datasplitpath=args.datasplitpath)
            testset = Prostate_2D(root=args.vali_root, suffix='vali', resize=args.size, transform=None,
                                  mlb=False, datasplitpath=args.datasplitpath)
        else:
...
def prepare_dataloder(args):
    if args.baseline:
        trainset, testset = prepare_set2D(args)
        meta_sampler = None
    else:
        trainset, metaset, testset = prepare_set2D_mlb(args)
        meta_sampler = None
    train_sampler = None
    test_sampler = None
...
def get_consistencyloss(pseudo_labels, insize, outsize, flipindex):
    AUG = 5
    b, c, w, h = pseudo_labels.shape[0], pseudo_labels.shape[1], pseudo_labels.shape[2], pseudo_labels.shape[3]
    loss_consistency = 0
    for i in range(b // AUG):
        pseudo = pseudo_labels[i * AUG:(i + 1) * AUG, :, :, :]
        zoom_in, zoom_outall, findex = insize[i], outsize[i], flipindex[i]
        ...
        p_2_ori = torch.zeros(reshape)
        p_2_ori[zoom_in:-zoom_in, zoom_in:-zoom_in] = p_2
        p_2 = cv2.resize(p_2_ori.cpu().detach().numpy(), [h, w])
        p_2 = torch.tensor(p_2).cuda()

        loss_consistency = loss_consistency + torch.mean((cur_pseudo - p_2) ** 2)

        p_3 = pseudo[2, :, :, :]
        zoom_out = zoom_outall[0]
        p_3_ori = p_3[0, zoom_out:-zoom_out, zoom_out:-zoom_out]
        p_3 = cv2.resize(p_3_ori.cpu().detach().numpy(), [h, w])
        p_3 = torch.tensor(p_3).cuda()

        loss_consistency = loss_consistency + torch.mean((cur_pseudo - p_3) ** 2)

        p_4 = pseudo[3, :, :, :]
        zoom_out = zoom_outall[1]
        p_4_ori = p_4[0, zoom_out:-zoom_out, zoom_out:-zoom_out]
        p_4 = cv2.resize(p_4_ori.cpu().detach().numpy(), [h, w])
        p_4 = torch.tensor(p_4).cuda()

        loss_consistency = loss_consistency + torch.mean((cur_pseudo - p_4) ** 2)
        ...
        if findex[0] == 3:
            ...
            p_5 = flip(p_5, findex[0])
        p_5 = torch.tensor(p_5).cuda()

        loss_consistency = loss_consistency + torch.mean((cur_pseudo - p_5) ** 2)

        cur_pseudo = pseudo[1, :, :, :]

        reshape = [h + 2 * zoom_in, w + 2 * zoom_in]
        img = cv2.resize(p_3.cpu().detach().numpy(), reshape)
        img = img[zoom_in:-zoom_in, zoom_in:-zoom_in]
        img = torch.tensor(img).cuda()

        loss_consistency = loss_consistency + torch.mean((cur_pseudo - img) ** 2)

        reshape = [h + 2 * zoom_in, w + 2 * zoom_in]
        img = cv2.resize(p_4.cpu().detach().numpy(), reshape)
        img = img[zoom_in:-zoom_in, zoom_in:-zoom_in]
        img = torch.tensor(img).cuda()
        ...
        reshape = [h + 2 * zoom_in, w + 2 * zoom_in]
        img = cv2.resize(p_5[0, :, :].cpu().detach().numpy(), reshape)
        img = img[zoom_in:-zoom_in, zoom_in:-zoom_in]
        img = torch.tensor(img).cuda()
        ...
        reshape = [h - 2 * zoom_out, w - 2 * zoom_out]
        imgr = cv2.resize(p_4.cpu().numpy(), reshape)
        img = np.zeros([w, h])
        img[zoom_out:-zoom_out, zoom_out:-zoom_out] = imgr
        img = torch.tensor(img).cuda()

        loss_consistency = loss_consistency + torch.mean((cur_pseudo - img) ** 2)

        reshape = [h - 2 * zoom_out, w - 2 * zoom_out]
        imgr = cv2.resize(p_5[0, :, :].cpu().numpy(), reshape)
        img = np.zeros([w, h])
        img[zoom_out:-zoom_out, zoom_out:-zoom_out] = imgr
        img = torch.tensor(img).cuda()
    ...
    return 2 * loss_consistency / (b // AUG) / ((AUG-1)*AUG)
    # return loss_consistency / (b // AUG)
